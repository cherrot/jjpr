...
use crate::config::StackNavigationMode;
...
pub fn execute_submission_plan(
    ...
    reviewers: &[String],
    dry_run: bool,
    stack_navigation_mode: StackNavigationMode,
) -> Result<()> {
    ...
    for bookmark in &plan.bookmarks_needing_push {
        if dry_run {
            println!(
                "  Would push bookmark '{}' to {}",
                bookmark.name, plan.remote_name
            );
            continue;
        }
    ...
    for item in &plan.bookmarks_needing_pr {
        ...
        let mut pr = match github.create_pr(
            owner,
            repo,
            &item.title,
            &item.body,
    ...
    for item in &plan.bookmarks_needing_base_update {
        if dry_run {
            println!(
                "  Would update {} base: {} -> {}",
                fk.format_ref(item.pr.number),
                item.pr.base.ref_name,
                item.expected_base
            );
            continue;
        }
    ...
    for item in &plan.bookmarks_needing_body_update {
        ...
        if let Some(pr) = bookmark_to_pr.get_mut(&item.bookmark.name) {
            set_pr_body(pr, item.new_body.clone());
        }
    ...
    for (bookmark, pr_number) in &plan.bookmarks_needing_reviewers {
        ...
        println!("  Requesting reviewers on {}...", fk.format_ref(*pr_number));
        ...
        completed_actions.push(format!(
            "Requested reviewers on {}",
            fk.format_ref(*pr_number)
        ));
    }

    // Phase 7: Sync stack navigation
    let navigation_updated = if dry_run {
        match stack_navigation_mode {
            StackNavigationMode::Description => {
                println!("  Would sync stack navigation in PR descriptions");
            }
            StackNavigationMode::Comment => {
                println!("  Would sync stack navigation in PR comments");
            }
        }
        0
    } else {
        match sync_stack_navigation(github, plan, &mut bookmark_to_pr, stack_navigation_mode) {
            Ok(n) => {
                if n > 0 {
                    println!(
                        "  Synced stack navigation on {n} {}.",
                        if n == 1 { "PR" } else { "PRs" }
                    );
                }
                n
            ...
            Err(e) => {
                eprintln!("  Warning: failed to sync stack navigation: {e}");
    ...
    if !plan.has_actions() && plan.bookmarks_already_merged.is_empty() && navigation_updated == 0 {
...
fn set_pr_body(pr: &mut PullRequest, body: String) {
    pr.body = if body.is_empty() { None } else { Some(body) };
}
...
fn merge_with_previous_entries(
    ...
    let current_by_name: HashMap<&str, &EntryData> =
...
fn sync_stack_navigation(
    ...
    match mode {
        StackNavigationMode::Description => update_stack_descriptions(github, plan, bookmark_to_pr),
        StackNavigationMode::Comment => {
            let removed = remove_stack_navigation_from_descriptions(github, plan, bookmark_to_pr)?;
            let updated = update_stack_comments(github, plan, bookmark_to_pr)?;
            Ok(removed + updated)
        }
    }
}

fn remove_stack_navigation_from_descriptions(
    github: &dyn Forge,
    plan: &SubmissionPlan,
    bookmark_to_pr: &mut HashMap<String, PullRequest>,
) -> Result<usize> {
        ...
        .filter(|b| b.name != plan.default_branch)
    {
        ...
        let new_body = comment::remove_stack_navigation(current_body);
        if new_body == current_body {
            continue;
        }

        github.update_pr_body(owner, repo, pr.number, &new_body)?;
        if let Some(pr) = bookmark_to_pr.get_mut(&bookmark.name) {
            set_pr_body(pr, new_body);
        }
        updated += 1;
    }
...
fn update_stack_descriptions(
    ...
    for bookmark in plan
        ...
        .filter(|b| b.name != plan.default_branch)
    {
        let Some(pr) = bookmark_to_pr.get(&bookmark.name).cloned() else {
            continue;
        };
        let current_body = pr.body.as_deref().unwrap_or("");

        if stack_bookmark_count <= 1 {
            let new_body = comment::remove_stack_navigation(current_body);
            if new_body != current_body {
                github.update_pr_body(owner, repo, pr.number, &new_body)?;
                if let Some(pr) = bookmark_to_pr.get_mut(&bookmark.name) {
        ...
        let previous_items = previous_stack_items(github, owner, repo, &pr)?;
        let merged = merge_with_previous_entries(&current_entries, &previous_items);
        let entries: Vec<StackEntry> = merged
            .iter()
            .map(|e| StackEntry {
            ...
            .collect();
        let stack_body = comment::generate_comment_body(&entries);
        let new_body = comment::upsert_stack_navigation(current_body, &stack_body);
...
fn previous_stack_items(
    github: &dyn Forge,
    owner: &str,
    repo: &str,
    pr: &PullRequest,
) -> Result<Vec<comment::StackCommentItem>> {
    if let Some(items) = pr
        .body
        .as_deref()
        .and_then(comment::extract_stack_navigation)
        .and_then(comment::parse_comment_data)
        .map(|data| data.stack)
    {
        return Ok(items);
    ...
    let comments = github.list_comments(owner, repo, pr.number)?;
    Ok(comment::find_stack_comment(&comments)
        .and_then(|c| c.body.as_deref())
        .and_then(comment::parse_comment_data)
        .map(|data| data.stack)
        .unwrap_or_default())
...
mod tests {
    ...
    use crate::forge::types::{
        ChecksStatus, IssueComment, MergeMethod, PrMergeability, PrState, PullRequestRef, RepoInfo,
        ReviewSummary,
    };
    use crate::jj::Jj;
    ...
    impl Forge for RecordingGitHub {
        ...
        fn request_reviewers(&self, _o: &str, _r: &str, n: u64, revs: &[String]) -> Result<()> {
            self.calls
                .lock()
                .expect("poisoned")
    ...
    fn test_dry_run_produces_no_side_effects() {
        ...
        execute_plan(&jj, &github, &plan, &[], true).unwrap();
    ...
    fn test_creates_pr_with_correct_base() {
        ...
        execute_plan(&jj, &github, &plan, &[], false).unwrap();
    ...
    fn test_requests_reviewers_on_new_prs() {
        ...
        execute_plan(&jj, &github, &plan, &reviewers, false).unwrap();
    ...
    fn test_single_pr_skips_stack_comment() {
        ...
        execute_plan_with_comments(&jj, &github, &plan, &[], false).unwrap();
    ...
    fn test_two_prs_creates_stack_comments() {
        ...
        plan.bookmarks_needing_pr
            .push(super::super::plan::BookmarkNeedingPr {
    ...
    #[test]
    fn test_two_prs_syncs_stack_navigation_in_descriptions_by_default() {
        let jj = RecordingJj::new();
        let github = RecordingGitHub::new();
        ...
        let body_updates: Vec<_> = calls
            .iter()
            .filter(|c| c.starts_with("update_pr_body"))
            .collect();
        assert_eq!(
            body_updates.len(),
            2,
            "should update both PR descriptions: {calls:?}"
        );
        assert!(
            !calls.iter().any(|c| c.starts_with("create_comment")),
            "description mode should not create stack comments: {calls:?}"
        );
    }
    ...
    #[test]
    fn test_comment_mode_removes_stack_navigation_from_description() {
        let jj = RecordingJj::new();
        let github = RecordingGitHub::new();
        let body = comment::upsert_stack_navigation("Managed body", "Stack info");
        ...
        assert!(
            ...
            "comment mode should remove stack navigation from description"
        );
    }
    ...
    fn test_dry_run_skips_body_update() {
        ...
        assert!(
            !github
                .calls()
                .iter()
                .any(|c| c.starts_with("update_pr_body")),
    ...
    fn test_create_pr_as_draft() {
        ...
        assert!(
            github
                .calls()
                .iter()
                .any(|c| c.starts_with("create_draft_pr:")),
    ...
    fn test_requests_reviewers_on_existing_prs() {
        ...
        assert!(
            github
                .calls()
                .iter()
                .any(|c| c == "request_reviewers:#10:alice"),
    ...
    fn test_skips_already_requested_reviewers() {
        ...
        assert!(
            github
                .calls()
                .iter()
                .any(|c| c == "request_reviewers:#10:alice,bob"),
    ...
    fn test_partial_failure_reports_completed_actions() {
        ...
        let err = execute_plan(&FailingJj, &github, &plan, &[], false).unwrap_err();
    ...
    fn test_dry_run_skips_reviewer_requests_on_existing() {
        ...
        let reviewers = vec!["alice".to_string()];
        execute_plan(&jj, &github, &plan, &reviewers, true).unwrap();
    ...
    fn test_stack_comment_excludes_default_branch() {
        ...
        assert_eq!(
            calls.len(),
            2,
            "should create exactly two comments: {calls:?}"
        );
        ...
        for body in bodies.iter() {
            assert!(
                !body.contains("`main`"),
                "comment should not contain main: {body}"
            );
        }
    }
    ...
    fn test_comment_failure_does_not_abort() {
        ...
        let result = execute_plan_with_comments(&jj, &CommentFailsGitHub, &plan, &[], false);
        assert!(
            result.is_ok(),
            "comment failure should not abort: {result:?}"
        );
    }
    ...
    fn test_merge_previous_entries_preserves_removed() {
        ...
        let previous = vec![
            ...
            comment::StackCommentItem {
                bookmark_name: "B".into(),
                pr_url: "url_b".into(),
                pr_number: 2,
                is_merged: false,
            },
            comment::StackCommentItem {
                bookmark_name: "C".into(),
                pr_url: "url_c".into(),
                pr_number: 3,
                is_merged: false,
            },
